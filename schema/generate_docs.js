...
async function main() {
    ...
    for (const [name, type] of seamarksNs.models) {
        ...
        const { description: doc, sources } = getDoc(program, type);
        ...
        while (currentType) {
            for (const [propName, prop] of currentType.properties) {
                ...
                if (propName === "geometryType") {
                    ...
                    const { description: propDoc } = getDoc(program, prop);
                    attributes.push({
                        name: propName,
    ...
    for (const m of models) {
        ...
        for (const attr of m.attributes) {
            let osmTagDisplay = "-";
            if (attr.osmTags && attr.osmTags.length > 0) {
                osmTagDisplay = attr.osmTags.map(t => `\`${t}\``).join(", ");
