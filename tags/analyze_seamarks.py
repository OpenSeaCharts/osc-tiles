...
def analyze_seamarks(csv_file, config_file, lua_file):
                ...
                if key == 'seamark:type':
                    for val in row[2:]:
                        if val and val != '...':
                            # Validation logic
                            # 1. Must be lowercase
                            if not val.islower() and not val.replace('_', '').isdigit(): # 5_knots is ok
                                # Check if it's just numbers? No.
                                # Allow 5_knots.
                                # Reject Landmark, Bridge.
                                if not val[0].isdigit(): # 5_knots starts with digit
                                     if any(c.isupper() for c in val):
                                         continue

                            # 2. Must not contain invalid chars
...
if __name__ == "__main__":
    ...
    analyze_seamarks(csv_path, config_path, lua_path)
